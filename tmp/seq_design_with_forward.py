...
class FullAtomDataset():
    ...
    def __getitem__(self, data_name, strict_mode = True, crop = True):
        ...
        try:
            ...
            modefied_pdb_tensor,status = align_pdb_pdbtm_tensors(pdb_tensor_list,pdbtm_tensor_list_tensor,features['pdbname'])
            if not status:
                logger.warning(f"{features['pdbname'] } bad sequence align!")
...
def make_prev_token_from_t(B, L, lig_mask, device, merged_s):
    ...
    skeptical_mask_prot = torch.ones_like(output_scores,dtype=torch.bool)
            ...
            for l_idx, f_ in enumerate(npy_list):
                ...
                if not f_.endswith('.npy'):
                    continue
                        ...
                        features = full_atom_dataset.__getitem__(data_name[:4], strict_mode = True, crop = True)
                        ...
                        randn = torch.zeros_like(merged_chain_mask)
                        ...
                        # # pdbtm_regions = torch.zeros_like(merged_s)
                        # # pdbtm_regions = torch.nn.functional.pad(pdbtm_regions, (0, (merged_s.shape[1]-pdbtm_regions.shape[1])))#默认第二个维度是L
                        ...
                        # logger.info(f"mask_mode :{mask_mode}")
                        # logger.info(f"pdbtm_regions:{pdbtm_regions}")
                        last_iter_null_embed = torch.zeros((B, L, 1280)).to(device)
                        last_prev_tokens = make_prev_token_from_t( B, L, merged_lig_mask, device, merged_s)
                        args_dict = {
                            'merged_coords': merged_coords[0],
                            'merged_s': merged_s[0],
                            'merged_mask': merged_mask[0],
                            'merged_chain_mask': merged_chain_mask[0],
                            'merged_residue_idx': merged_residue_idx[0],
                            'merged_chain_encoding_all': merged_chain_encoding_all[0],
                            # 'randn': randn[0],
                            'lig_node_attr': lig_node_attr[0],
                            'lig_edge_attr': lig_edge_attr[0],
                            'lig_edge_index': lig_edge_index[0],
                            'use_input_decoding_order': False,
                            'decoding_order': None,
                            'lig_mask': merged_lig_mask[0],
                            'seq_mask': None,
                            'esm_embedding': esm_embedding,
                            'last_iter_S_embed': last_iter_null_embed[0],
                            'unimol_reprs': unimol_reprs[0],
                            'torsion_angles': torsion_angles[0],
                            'alt_chi_angles': alt_chi_angles[0],
                            'torsion_angle_mask': torsion_angle_mask[0],
                            'prev_tokens': last_prev_tokens[0],
                            'plip_anno_itype_list': plip_anno_itype_list[0]
                        }
                        with open('forward_ctx_args.txt', 'w', encoding='utf-8') as f:
                            for name, value in args_dict.items():
                                # 1) 打印参数名
                                f.write(f'### {name}\n')
                                # 2) 把 tensor / numpy 展平成一维
                                if isinstance(value, (torch.Tensor, np.ndarray)):
                                    flat = value.detach().cpu().numpy().flatten()
                                else:
                                    # 普通 Python 对象先转 list，再展平
                                    flat = np.array(value).flatten()
                                # 3) 逐行写元素
                                for v in flat:
                                    f.write(f'{v}\n')
                                f.write('\n')
                        # 3. 把 tensor / numpy 转成可序列化的 Python 对象
                        ...
                        # serial_dict = {k: to_serializable(v) for k, v in args_dict.items()}

                        # # 4. 写入 txt（人类可读）
                        # with open('design_ctx_args.txt', 'w', encoding='utf-8') as f:
                        #     for k, v in serial_dict.items():
                        #         f.write(f'{k}: {v}\n')

                        print('已写入 forward_ctx_args.txt')


                        logits, src_token, log_sc_logits, prev_torsion_mask, pdbtm_regions_mask,edge_mem_mask = designer(
                            merged_coords,
                            merged_s,
                            merged_mask,
                            merged_chain_mask,
