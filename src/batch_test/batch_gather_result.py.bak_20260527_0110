...
def extract_all_headers(input_fasta_path):
...
def gather_simple_results(batch_size, iter_number = 20, root_dir = ''): #root_dir精确到pdbname
    ...
    # Protein name validation removed - using directory name directly
    ...
    all_records = []
    template = []
    n_per_file = math.ceil(batch_size / len(fastas))
    for idx, fa in enumerate(fastas):
        if idx == 0:
            template = read_template(fa)
        all_records.extend(read_last_n(fa, n_per_file))


    all_records = [
            (f">{code}_design_{i}_{iter_number}; {'; '.join(h.lstrip('>').split(';')[1:]).strip()}", s)
            for i, (h, s) in enumerate(all_records)
        ]
...
def gather_results(root_dir):
    ...
    for sub in abs_dirs:
        ...
        header_list = extract_all_headers(merged_file)
        header_list = header_list[1:] #去掉模板位置
        # print(header_list)

        header_dict = []
        for h in header_list:
            parts = h.replace(',chains', ';chains').split(';')
            # Handle both 4-part (old) and 5-part (with sign) header formats
            if len(parts) >= 5:
                # Format: name; sign; overall; pocket; chains
                header_dict.append({
                    'name': parts[0].strip(),
                    'overall_identity': float(parts[2].split('=')[1]),
                    'pocket_identity': float(parts[3].split('=')[1]),
                    'chains': parts[4].split('=')[1],
                })
            elif len(parts) >= 4:
                # Format: name; overall; pocket; chains (no sign)
                header_dict.append({
                    'name': parts[0].strip(),
                    'overall_identity': float(parts[1].split('=')[1]),
                    'pocket_identity': float(parts[2].split('=')[1]),
                    'chains': parts[3].split('=')[1],
                })
...
def gather_mpnn_results(root_dir):
    ...
    for merged_file in os.listdir(root_dir):
        ...
        header_list = extract_all_headers(merged_path)
        header_list = header_list[1:] #去掉模板位置
        # print(header_list)
