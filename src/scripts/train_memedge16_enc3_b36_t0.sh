#!/bin/bash
# ------------- memedge16 b36 重训: 有效batch 3x12=36 + valid时间步固定0 + mem_edge16架构 + encoder 3层解冻(PIfold保持冻结) 
GPU=${GPU:-0}
tm_raw="zero_afdb_with_0124afdb_cluster_dict_1_3_b36_t0_memedge16_enc3"
...
run_training() {
      ...
      CUDA_VISIBLE_DEVICES=$GPU /home/chenty/miniconda3/envs/abacust/bin/fairseq-train \
            ...
            --tm_raw $tm_raw \
            --mem_config /home/chenty/abacust_mem/src/scripts/configs/mem_config_memedge16_enc3.yaml \
            2>&1 | tee -a "$log_file"
}
