...
def main(args):
    ...
    # order_suffix = args.order_suffix
            ...
            if not f_.endswith('.npy'):
                ...
                try:
                    ...
                    for index in range(epochs):
                        ...
                        valid_pdbtm_regions = pdbtm_data_item.extract_full_region_np()
                        pdbtm_regions = torch.tensor(valid_pdbtm_regions)

                        if args.tm_raw.startswith("zero") or args.tm_raw.startswith("raw"):
                            pdbtm_regions = torch.zeros_like(merged_s)
                            G = torch.nn.Parameter(torch.cat([torch.eye(3,3, device=device).unsqueeze(0).repeat(single_batchsize,1,1),
                                    torch.zeros(single_batchsize,3,1, device=device)], dim=2))   # [B,3,4]
                            N =torch.zeros(single_batchsize, 1, 3, device=device)
                        elif args.tm_raw.startswith("pdbtm"):
                            pdbtm_regions = torch.zeros_like(merged_s)
                        else:
                            # pdbtm_regions,G,N = get_pdbtm_data(batch_size=single_batchsize, align = merged_s, device=device)
                            pass
                    ...
                    #########################################################################
                    # 将不同批次的聚合起来
                    from batch_gather_result import gather_simple_results
                    gather_simple_results(batch_size= single_batchsize , iter_number=iter_num, root_dir=subdir)
                    #######################################################################
