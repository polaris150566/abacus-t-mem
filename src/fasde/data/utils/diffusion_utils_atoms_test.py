...
def make_complex_feature(
    ...
    rec_atom_positions = torch.concat([torch.zeros(1,37,3), rec_atom_positions,torch.zeros(1,37,3)])
    rec_atom_mask = torch.concat([torch.zeros(1,37), rec_feature['all_atom_mask'],torch.zeros(1,37)])
    ...
    aatype = torch.cat([torch.tensor(33)[None], rec_feature['aatype']+4, torch.tensor(25)[None], torch.LongTensor([24]*lig_len)], dim=0)
    node_mask = torch.ones((frame_len,))
    shuffle_index = np.arange(frame_len)
    inverse_shuffle_index = shuffle_index
    # coords = torch.concat([torch.zeros(1,3,3), all_atom_positions[:,:3], torch.zeros(1,3,3)], dim=0)
    coords = all_atom_positions[:,:3]
    # residx = residx.numpy()
    # residx = [1] + list(residx - np.min(residx) + 2) + [np.max(residx) -np.min(residx) + 3]
    # residx = np.asarray(residx, dtype=np.int64)
    # residx = torch.from_numpy(residx)
    tokens = aatype
    atom_mask = all_atom_mask[:,:3]
    # atom_mask = torch.concat([torch.ones(1,3), all_atom_mask[:,:3], torch.ones(1,3)],dim=0)
    # single = torch.concat([torch.zeros(1,single.shape[-1]), single, torch.zeros(1,single.shape[-1])], dim=0)
...
def make_protein_feature(
    ...
    tokens = aatype
    atom_mask = all_atom_mask[:,:3]
    if residx.shape[0]!=coords.shape[0]:
