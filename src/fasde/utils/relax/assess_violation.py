...
from fasde.modules.alphafold import folding
...
def find_violations(prot_np: protein.Protein, clash_overlap_tolerance=1.5):
  ...
  violations = folding.find_structural_violations(
      ...
      config=ml_collections.ConfigDict(
          {"violation_tolerance_factor": 12,  # Taken from model config.
           "clash_overlap_tolerance": clash_overlap_tolerance,  # Taken from model config.
          }))
...
def get_violation_metrics(prot: protein.Protein, clash_overlap_tolerance=1.5):
  """Computes violation and alignment metrics."""
  structural_violations, struct_metrics = find_violations(prot, clash_overlap_tolerance)
