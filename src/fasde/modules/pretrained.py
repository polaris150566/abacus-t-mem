...
def load_regression_hub(model_name):
    try:
        url = f"/train14/superbrain/lhchen/protein/pretrain/esm2/params/{model_name}-contact-regression.pt"
        # regression_data = load_hub_workaround(url)
        regression_data = torch.load(url, map_location="cpu")
    except:
        print('No regression data loaded!')
        regression_data = None
    return regression_data


def _download_model_and_regression_data(model_name):
    url = f"/train14/superbrain/lhchen/protein/pretrain/esm2/params/{model_name}.pt"
    # model_data = load_hub_workaround(url)
    model_data = torch.load(url, map_location="cpu")
