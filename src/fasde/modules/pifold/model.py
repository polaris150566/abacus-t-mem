...
class PiFoldModel(nn.Module):
    ...
    def encode(self, X, mask, lengths, S=None):
        ...
        # h_V = self.node_embedding(h_V.half())
        h_V = self.norm_nodes(h_V)
        h_V = self.W_v(h_V)

        # h_P = self.W_e(self.norm_edges(self.edge_embedding(h_P.half())))
    ...
    def _full_dist(self, X, mask, top_k=30, eps=1E-6):
        """根据X的坐标与topk，选定最近的节点的坐标以及，节点的idx


        Args:
            X (_type_): _description_
            mask (_type_): _description_
            top_k (int, optional): _description_. Defaults to 30.
    ...
    def _get_features(self, S, score, X, mask, lengths):
        ...
        Args:
            S         (B, N)            氨基酸类别索引
            score     (B, N)            氨基酸置信度/打分
            X         (B, N, 4, 3)      每个残基 4 个主链原子 (N, Ca, C, O) 的 3-D 坐标
            mask      (B, N)            0/1，1 表示该残基真实存在
            lengths   (B,)              每条序列的真实长度

        Returns:
            _X         (M, 4, 3)         经过 mask 过滤后的原子坐标
            S         (M,)              过滤后的氨基酸索引
            score     (M,)              过滤后的置信度
            _V        (M, d_V)          节点特征
            _E        (E, d_E)          边特征
            E_idx     (2, E)            COO 格式的边索引 [dst, src]
            batch_id  (M,)              每个节点属于哪条原始 batch
            mask_bw   (E,)              解码顺序的“向后看”掩码
            mask_fw   (E,)              解码顺序的“向前看”掩码
            decoding_order (M,)         节点解码顺序（全局索引）
        """
        device = X.device
        mask_bool = (mask==1)
        ...
        # 用 Ca 坐标算全连接距离，取 top-k 邻居 → (D_neighbors, E_idx)。
        ...
        # 随机扰动 + 排序得到 decoding_order。
        # 构造 order_mask_backward，用于自回归掩码 mask_bw / mask_fw。
        ...
        b_fp32 = b.float()
        c_fp32 = c.float()
        a_fp32 = torch.cross(b_fp32, c_fp32, dim=-1)
        a = a_fp32.to(b.dtype)
