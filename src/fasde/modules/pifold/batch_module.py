...
#################################### node modules ###############################
class NeighborAttention(MessagePassing):
    ...
    def forward(self, h_V, h_E, center_id, batch_id, dst_idx=None):
        ...
        x = self.propagate(edge_index, x=x, W=attend*V)
...
class NeighborAttentionBatch(nn.Module):
    def __init__(self, num_hidden, num_in, num_heads=4, edge_drop=0.0, output_mlp=True):
        super(NeighborAttentionBatch, self).__init__()
        self.num_heads = num_heads
        self.num_hidden = num_hidden
        self.edge_drop = edge_drop
        self.output_mlp = output_mlp
        ...
        self.register_buffer("masked_bias", torch.tensor(-1e12))

    def forward(self, h_V_i, h_EV_j, edge_mask):
        # (Pdb) h_V_i.shape
        # torch.Size([1, 265, 48, 128])
        # (Pdb)  h_EV_j.shape
        # torch.Size([1, 265, 48, 256])

        B, N, K = h_EV_j.shape[:3]#B，L K = 48
        n_heads = self.num_heads # 4
        d = int(self.num_hidden / n_heads)

        # import pdb;pdb.set_trace()


        w = self.Bias(torch.cat([h_V_i, h_EV_j],dim=-1)).view(B, N, K, n_heads, 1) #([B, 265, 48, 4, 1])，最后的view相当于给扩了一个维度
        attend_logits = w/np.sqrt(d)
        attend_logits = torch.where(edge_mask[..., None, None], attend_logits, self.masked_bias)
        attend = F.softmax(attend_logits.float(), dim=2) # [B x N x K x h x H] #按照edgemask将需要掩盖的边去掉，之后按照邻居所在的维度进行softmax，表示一个节点接受周围节点消息传递的比例

        V = self.W_V(h_EV_j).view(B, N, K, n_heads, d)
        h_V = torch.sum(attend*V, 2).view([B, N, self.num_hidden])#沿着第二个维度求和（相当于将消息从边汇聚到节点），之后恢复原来的维度

        if self.output_mlp:#目前是true
            h_V_update = self.W_O(h_V) #对融合的信息进行linear整合后返回
        else:
            h_V_update = h_V
        return h_V_update
...
class EdgeMLPBatch(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        super(EdgeMLPBatch, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.dropout = nn.Dropout(dropout)
        self.norm = BatchNorm(num_hidden)
        self.W11 = nn.Linear(num_hidden + num_in, num_hidden, bias=True)
        self.W12 = nn.Linear(num_hidden, num_hidden, bias=True)
        self.W13 = nn.Linear(num_hidden, num_hidden, bias=True)
        self.act = torch.nn.GELU()

    def forward(self, h_V, h_E, edge_idx, edge_mask=None):
        # h_V.sahpe [B x L x H]
        # h_E.shape [B x L x K x H]

        h_V_j = gather_nodes(h_V, edge_idx)
        h_V_i = h_V.unsqueeze(-2).expand(-1,-1, h_V_j.size(-2),-1)

        h_EV = torch.cat([h_V_i, h_E, h_V_j], dim=-1)
        h_message = self.W13(self.act(self.W12(self.act(self.W11(h_EV)))))#一轮消息聚合之后使用mlp进行整合
        if edge_mask is not None:
            h_message = edge_mask.unsqueeze(-1) * h_message
        h_E = self.norm(h_E + self.dropout(h_message))#使用resnet加上dropout之后返回
        return h_E
...
class ContextBatch(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, node_context = False, edge_context = False):
        super(ContextBatch, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.node_context = node_context
    ...
    def forward(self, h_V, h_E, edge_idx, edge_mask):
        # h_V.sahpe [B x L x H]
        # h_E.shape [B x L x K x H]

        if self.node_context:
            c_V = torch.mean(h_V, dim=[-2], keepdim=True)
...
class GeneralGNNBatch(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, node_net = 'AttMLP', edge_net = 'EdgeMLP', node_context = 0, edge_context = 0): #node是true，edge是false
        super(GeneralGNNBatch, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.ModuleList([BatchNorm(num_hidden) for _ in range(3)])
        self.node_net = node_net
        self.edge_net = edge_net
        if node_net == 'AttMLP':
            self.attention = NeighborAttentionBatch(num_hidden, num_in, num_heads=4)

        if edge_net == 'None':
            pass
        if edge_net == 'EdgeMLP':
            self.edge_update = EdgeMLPBatch(num_hidden, num_in, num_heads=4)

        self.context = ContextBatch(num_hidden, num_in, num_heads=4, node_context=node_context, edge_context=edge_context)#其实就是对输入的h_V做个均值（-2维）之后搞了个MLP，h_E都没动
    ...
    def forward(self, h_V, h_E, edge_idx, node_mask, edge_mask):
        # h_V.sahpe [B x L x H]
        # h_E.shape [B x L x K x H]

        mask_V = node_mask.unsqueeze(-1)
        # 掩码

        h_V = mask_V * h_V
        h_V_j = gather_nodes(h_V, edge_idx)
        h_EV_j = torch.cat([h_E, h_V_j], -1)
        h_V_i = h_V.unsqueeze(-2).expand(-1,-1, h_V_j.size(-2),-1)#这个相当于是接受信息的节点
        dh = self.attention(h_V_i, h_EV_j, edge_mask)#h_EV_j相当于是准备将信息传递的节点，包括之前聚合好的周围的节点以及边

        h_V = self.norm[0](h_V + self.dropout(dh))
        dh = self.dense(h_V)
        h_V = self.norm[1](h_V + self.dropout(dh))

        #对hv进行attention之后更新hv，之后对hv进行mlp之后resnet之后再更新一遍

        h_E = self.edge_update(h_V, h_E, edge_idx, edge_mask )
        #包含将hv和he进行消息聚合之后mlp处理

        mask_V = node_mask.unsqueeze(-1)
        h_V = mask_V * h_V
        #掩码

        h_V, h_E = self.context(h_V, h_E, edge_idx, edge_mask )
        # 对hv均值之后做了一个mlp
...
class StructureEncoder(nn.Module):
    ...
    def forward(self, h_V, h_P, P_idx, node_mask, edge_mask):

        for layer in self.encoder_layers:
            h_V, h_P = layer(h_V, h_P, P_idx, node_mask, edge_mask)
