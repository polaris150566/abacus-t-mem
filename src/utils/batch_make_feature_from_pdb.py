...
ignore_mol_type = [
    ...
    "PGW", "IOD", "3NI", "ZRW", "78M", "UNX", "MES", "CCN", "PO4", "HOH"]
...
def get_args():
    ...
    parser.add_argument("--pdb_chain_file", type=str)
    parser.add_argument("--process_worker", type=int, default=90)

    args = parser.parse_args()
    return args


def make_lig(pdbcode_chain, outdir):
    prot_lig_pdb_f, protein_chain = pdbcode_chain.split()[:2]
    ...
    process(prot_lig_pdb_f, lig_dir)
        
        
def batch_make_lig(args):
    outdir = args.out_dir
    pdb_chain_file = args.pdb_chain_file
    
    with open(pdb_chain_file, 'r') as reader:
        all_lines = reader.readlines()
    
    Parallel(args.process_worker)(delayed(make_lig)(pdbcode_chain, outdir) for pdbcode_chain in tqdm(all_lines,total=len(all_lines)))
    
    
def make_complex_from_pdb(pdbcode_chain, outdir):
    prot_lig_pdb_f, protein_chain = pdbcode_chain.split()[:2]
    prot_lig_pdbname = os.path.basename(prot_lig_pdb_f).split('.')[0]
    npy_f = f'{outdir}/all_npy/{prot_lig_pdbname}_{protein_chain}.npy'
    if os.path.isfile(npy_f):
        return
    os.makedirs(f'{outdir}/all_npy', exist_ok=True)
    lig_dir = f'{outdir}/all_lig/{prot_lig_pdbname}'
    # lig_dir = f'/database/lyf_database/all_pdb20230101_20240112_lig/{prot_lig_pdbname[1:3]}/{prot_lig_pdbname}'
    os.makedirs(lig_dir, exist_ok=True)
    # process(prot_lig_pdb_f, lig_dir)
...
def batch_make_complex(args):
    ...
    Parallel(args.process_worker)(delayed(make_complex_from_pdb)(pdbcode_chain, outdir) for pdbcode_chain in tqdm(all_lines,total=len(all_lines)))
    
    
def make_unimol_rep(args):
    outdir = args.out_dir
    pdb_chain_file = args.pdb_chain_file
    ...
    for pdbcode_chain in tqdm(all_lines):
        prot_lig_pdb_f, protein_chain = pdbcode_chain.split()[:2]
        prot_lig_pdbname = os.path.basename(prot_lig_pdb_f).split('.')[0]
        npy_f = f'{outdir}/all_npy/{prot_lig_pdbname}_{protein_chain}.npy'
        if os.path.isfile(npy_f):
            prot_lig_complex_dict = np.load(npy_f, allow_pickle=True).item()
            new_prot_lig_complex_dict = deepcopy(prot_lig_complex_dict)
            for ligname, lig_ctx in prot_lig_complex_dict['lig'].items():
                mol = lig_ctx['rdkit_ligand']
                cur_lig_graph = get_lig_graph(mol)
                unimol_reprs_dict = get_unimol_repr_from_pos(cur_lig_graph['ligand'], mol)

                if (cur_lig_graph['ligand'].pos.shape[0] == unimol_reprs_dict['atomic_reprs'][0].shape[0]):
                    new_prot_lig_complex_dict['lig'][ligname].update({'unimol_reprs': unimol_reprs_dict})
                else:
                    new_prot_lig_complex_dict['lig'].pop(ligname)
                    print(f'{ligname} wrong unimol process')
                    
            np.save(npy_f, new_prot_lig_complex_dict)
...
if __name__ == '__main__':
    args = get_args()
    batch_make_lig(args)
    batch_make_complex(args)
    make_unimol_rep(args)
