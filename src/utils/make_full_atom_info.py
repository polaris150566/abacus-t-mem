...
# from scuba_pipline import SD_pipline, fix_edge_index
...
def nodewraprun(mmCIF_dir, protein_chain, in_dir, out_dir, protein, logfile):
    try:
        print(f"protein: {protein} is processing ...")
        # /ps2/hyperbrain/yfliu25/cath_dataset/split_data/r2/1r26A00A/1r26A00.pdb
        new_dir = mmCIF_dir.split("/")[-1].split(".")[0]
        
        out_tmp_parroot = os.path.join(out_dir, new_dir)
        in_tmp_parroot = os.path.join(in_dir, new_dir)
        
        if not os.path.isdir(out_tmp_parroot):
            os.mkdir(out_tmp_parroot)

        in_data_file = os.path.join(in_tmp_parroot, "integrate_all.npz")
        out_data_file = os.path.join(out_tmp_parroot, "integrate_all.npz")

        add_all_atoms(
            pdbfile=mmCIF_dir, filetype="pdb",
            chain=protein_chain, out_data_file=out_data_file, in_data_file=in_data_file)
        print(out_data_file, 'add sidechain info')
...
def add_all_atoms(pdbfile, filetype, chain, out_data_file, in_data_file):
    ...
    tmpdata = np.load(in_data_file, allow_pickle=True)
    data_dict = tmpdata[str(tmpdata.files[0])].item()
    data_dict['full_atoms'] = {
        'aatype': encoded_sequence,
        'all_atom_positions': all_atom_positions,
        'all_atom_mask': all_atom_mask
    }
    np.savez(out_data_file, data_dict)


def multiprocess_run(proteinfile: str, in_root: str, out_root: str, mmCIF_root: str,
    ...
    for protein in protein_list:
        ...
        mmCIF_dir = os.path.join(mmCIF_root, protein_name[1:3], protein_name+".pdb")

        if merge_dssp_: dssp_dir = os.path.join(dssp_root, protein_name[1:3], protein_name+".dssp")
        else: dssp_dir = None

        out_dir = os.path.join(out_root, protein_name[1:3])
        in_dir = os.path.join(in_root, protein_name[1:3])
        ...
        if not overrun:
            ...
            if not os.path.isfile(os.path.join(in_dir, protein, "integrate_all.npz")):
                continue

        # nodewraprun(mmCIF_dir, protein_chain, in_dir, out_dir, protein, logfile)

        pool.apply_async(func=nodewraprun,
                         args=(mmCIF_dir, protein_chain, in_dir, out_dir, protein, logfile, ))
...
if __name__ == "__main__":
    IN_ROOT = "/yrfs1/hyperbrain/lhchen/for_yfliu25/local_sd"
    ...
    logfile = "/train14/superbrain/yfliu25/protein_diffusion/utils/logger_cath_fix.log"
    ...
    time0 = time.time()
    multiprocess_run(proteinfile=protein_file, in_root = IN_ROOT, out_root=OUT_ROOT, mmCIF_root=mmCIF_ROOT,
