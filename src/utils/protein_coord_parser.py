...
from mmcif_parser import _get_header
...
def get_atom4_from_main_array(main_array):
    coords = main_array.reshape(-1,5,3)
    return np.concatenate([coords[:, :3], coords[:, -1][:, None]], -2)
...
class ProteinCoordsParser(object):
    def __init__(self, poteinfile: str, datatype: str=None, chain: str=None, pseudo_gly=True,
                 mergedsspin_=False, dsspfile=None, with_o_atoms=True, omit_mainatoms_missing=True, authchain_dssp=False, only_single_model=True, model_num=None):
        ...
        if datatype == "mmCIF" or datatype == "mmcif" or datatype == "cif":
            if self.only_single_model and (model_num is None):
                self.model_num = 1
            elif ((not self.only_single_model) and (model_num is None)):
                self.model_num = 1
            else:
        ...
        try:
            self.chain_crd_array = np.stack([np.stack(list(res.values())[:37]) for res in list(self.reduced_chain_crd_dicts.values())])
        ...
        self.sequence =  "".join(list(map(lambda chain_idx: \
            "".join(list(map( lambda res_values: ENCODENUM2AA[res_values["AA"]], self.chain_crd_dicts[chain_idx].values()))),
    ...
    #     """获取chaincrddicts中的每个氨基酸的绝对序号（从1开始）

    #     Args:
    #         chain_name (_type_, optional): _description_. Defaults to None.

    #     Returns:
    #         _type_: _description_
    #     """
    #     result= None
    #     result={rel2abs[0]:rel2abs[1] for rel2abs in \
    #         list( zip( self.chain_crd_dicts[chain_name].keys(), np.arange(len(self.chain_crd_dicts[chain_name].keys())) )) }
    #     if result == None:
    #         return None
    #     else:
    #         return result
    def get_pdbresID2absID(self, chain_name=None, add_gap = 200):
        """获取chaincrddicts中的每个氨基酸的绝对序号（从1开始），并返回序列 index 数组和链掩码
        其中resid是直接将pdb获取过来的，如果有没有结构的氨基酸则会跳过该序号（也就是链内产生gap），链之间没有gap，且不同链之间的idx关系由原先的pdb文件决定
        而absid是直接按照顺序累加起来，且链之间加了gap

        Args:
            chain_name (str, optional): 链名称。 Defaults to None.

        Returns:
            tuple: (序列 index 数组, 链掩码数组)
        """
        seq_index = []
        abs_index = []
        chain_mask = []
        ...
        if chain_name is None:
            # 获取所有链的名称并按字母顺序排序
            chain_names = sorted(self.chain_crd_dicts.keys())
            # print('self.chain_crd_dicts.keys:')
            # print(self.chain_crd_dicts.keys())
            abs_id = 1  # 绝对序号初始化为1

            for i, chain in enumerate(chain_names):
                # 获取当前链的相对序号列表
                rel_ids = list(self.chain_crd_dicts[chain].keys())
                # 为当前链的每个相对序号生成绝对序号映射，并添加链名前缀
                for rel_id in rel_ids:
                    abs_index.append(abs_id)
                    seq_index.append(rel_id)
                    chain_mask.append(chain)
                    abs_id += 1
                # 如果不是最后一个链且需要添加间隙，则添加间隙
                if add_gap and i != len(chain_names) - 1:
                    abs_id += add_gap


            return np.array(abs_index), np.array(seq_index), np.array(chain_mask)
        else:
            # 如果指定了链名称，只处理该链
            try:
                rel_ids = list(self.chain_crd_dicts[chain_name].keys())
                seq_index = [idx + 1 for idx, _ in enumerate(rel_ids)]
                chain_mask = [chain_name] * len(rel_ids)
                return np.array(seq_index), np.array(chain_mask)
            except KeyError:
                print(f"链 {chain_name} 不存在")
                return None


    def get_atom_crd(self, atom_name: str) :
    ...
    def _parser_crd_dict4pdb(self, chain_ids, pseudo_gly=True):
        ...
        models_list = {str(model_ctx).split('=')[1].split('>')[0]: model_ctx for model_ctx in list(structure.get_models())}
        # if ( (self.only_single_model) and (len(models_list) != 1) ):
        #     raise FileNotFoundError(
        #         f"Only single model PDBs are supported. Found {len(models_list)} models."
        #     )

        # 如果设置为仅处理单模型且模型数量超过一个，则只取第一个模型
        if self.only_single_model and len(models_list) > 1:
            models_list = {list(models_list.keys())[0]: list(models_list.values())[0]}

        # 如果没有设置仅处理单模型，但模型数量超过一个，也只取第一个模型
        elif not self.only_single_model and len(models_list) > 1:
            models_list = {list(models_list.keys())[0]: list(models_list.values())[0]}

        model = models_list[str(self.model_num)]

        chain_ids_init = chain_ids
        if chain_ids_init is None:
            # chain_ids = list(map(lambda x: str(x).split('=')[1][0], model.child_list) )
            chain_ids = sorted(list(map(lambda x: str(x).split('=')[1][0], model.child_list) ))
            chain_ids_list = []
    ...
    def _parser_crd_dict4cif(self, chain_ids, gzfile=False, pseudo_gly=True):
        ...
        with warnings.catch_warnings():
            ...
            header = _get_header(pdb_dict)
            self.resolution = header['resolution']
            self.release_data = header['release_date']
            self.exp_method = header['structure_method']
            all_chain_crd_dicts = {}
            try:
                ...
                new_dict = {"ATOM": pdb_dict["_atom_site.group_PDB"], "atom_id": pdb_dict["_atom_site.label_seq_id"],
                            "chain": pdb_dict["_atom_site.auth_asym_id"],
                            "AA_type": pdb_dict["_atom_site.label_comp_id"], "icode": pdb_dict["_atom_site.pdbx_PDB_ins_code"],
                            "Atom_type": pdb_dict['_atom_site.label_atom_id'], "X": pdb_dict["_atom_site.Cartn_x"],
                            "Y": pdb_dict["_atom_site.Cartn_y"], "Z": pdb_dict["_atom_site.Cartn_z"],
            ...
            # df = df[df["model_num"] == model_list[self.model_num]]
            df = df[df['AA_type'].isin( list(PROTEINLETTER3TO1.keys()) )]
            df['ATOM'] = 'ATOM'

            model_list = sorted(list(set(df['model_num'].astype(np.int_).tolist())))
            ...
            df = df[df["model_num"].astype(np.int_) == int(self.model_num)]

            chain_ids_init = chain_ids
            if chain_ids_init is None:
                # chain_ids = list(set(df["chain"]))
                chain_ids = sorted(list(set(df["chain"])))
                chain_ids_list = []
    ...
    def get_main_crd_dict(self, reduce_dict=True, chain_name=None):
    ...
    def get_raw_crd_dict(self) :
        return self.chain_crd_dicts


    def get_main_crd_array(self, chain_name=None):
    ...
    def get_all_crd_array(self, chain_name=None):
        """return main crd array"""
        if chain_name is None:
            return self.chain_crd_array
        else:
            return np.stack([np.stack(list(res.values())[:37]) for res in list(self.chain_crd_dicts[chain_name].values())])
    ...
    def get_kai_torsions(self, degrees = True) :
        chain_tors = {}
...
if __name__ == "__main__":
    ...
    # pdbinfPDB = PoteinCoordsParser("/train14/superbrain/yfliu25/dataset/sabdab/hcdrs_cluster_pdb_gly/e7/7e7x.pdb")
    # pdbinfPDB = PoteinCoordsParser("/train14/superbrain/lhchen/data/alphafold_database/data/491/491821-0/AF-B0F7I1-F1-model_v3.cif.gz")
    # pdbinfPDB = PoteinCoordsParser('/train14/superbrain/yfliu25/dataset/assemble/divided/zz/5zz9-assembly2.cif')
    # pdbinfPDB = PoteinCoordsParser('/train14/superbrain/lhchen/data/PDB/20220102/mmcif/zz/5zz9.cif')

    # pdbinfPDB = PoteinCoordsParser('/train14/superbrain/yfliu25/dataset/assemble/divided/mu/1muc-assembly1.cif', chain=['B-2', 'A-3'])

    #pdbinfPDB = PoteinCoordsParser('/train14/superbrain/lhchen/data/PDB/20220102/mmcif/pc/9pcy.cif', chain=['A'], only_single_model=False, model_num=2)
    import pdb; pdb.set_trace()
