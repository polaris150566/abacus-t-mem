...
from .base_logger import Logger as logger
...
class MolDataReader(object):
    ...
    def read_data(self, data=None, is_train=True, **params):
        ...
        if isinstance(data, str):
            ...
            # data = pd.DataFrame.from_dict(data).rename(columns={smiles_col: 'SMILES'})
            # if 'target' in data:
        ...
        # import pdb; pdb.set_trace()
        if data.__contains__('atoms') and data.__contains__('coordinates'):
            dd = {}
            dd['atoms'] = [data['atoms']]
