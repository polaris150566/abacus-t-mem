...
class ProteinCoordsParser(object):
    def __init__(self, poteinfile: str, datatype: str=None, chain: str=None, pseudo_gly=True,
        ...
        if datatype is not None:
            assert datatype in ["PDB", "pdb", "mmCIF", "mmcif", "cif", "gz", "GZ", "ent"]
        else:
            assert self.proteinname.split('.')[-1] in ["PDB", "pdb", "mmCIF", "mmcif", "cif", "gz", "GZ", "ent"]
        ...
        elif datatype == "PDB" or datatype == "pdb" or datatype == "ent":
            if self.only_single_model and (model_num is None):
                self.model_num = 0
            elif ((not self.only_single_model) and (model_num is None)):
                self.model_num = 1
    ...
    def _parser_crd_dict4pdb(self, chain_ids, pseudo_gly=True):
        ...
        for chain_id in chain_ids:
            ...
            for res in residuesf:
                ...
                if resname in PROTEINLETTER3TO1.keys() and res.get_full_id()[3][0] == ' ':
                    # print("natural aa")
