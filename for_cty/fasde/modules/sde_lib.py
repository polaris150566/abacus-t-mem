...
class VPSDE(SDE):
  ...
  def marginal_prob_coeff(self, t):
    log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    mean_coef = torch.exp(log_mean_coeff)
    std = torch.sqrt(1. - torch.exp(2. * log_mean_coeff))
...
class priorVPSDE(SDE):
  ...
  def sde(self, x, t, mu):
    beta_t = self.beta_0 + t * (self.beta_1 - self.beta_0)
    drift = -0.5 * beta_t[:, None, None] * (mu - x)
    diffusion = torch.sqrt(beta_t)
    return drift, diffusion

  def marginal_prob(self, x, t, mu):
    log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    mean_coeff = torch.exp(log_mean_coeff)
    mean_coeff = mean_coeff[..., None, None]
    mean = (1. - mean_coeff) * mu + mean_coeff * x
    std = torch.sqrt(1. - torch.exp(2. * log_mean_coeff))
    return mean, std

  def marginal_sample(self, x, t, mu):
    mean, std = self.marginal_prob(x, t, mu)
    n = torch.randn_like(x)
    xt = mean + std[..., None, None] * n
    return xt, n
...
class VESDE(SDE):
  ...
  def marginal_prob_coeff(self, t):
    ...
    return  torch.ones_like(std), std
