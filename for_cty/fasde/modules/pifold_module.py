...
class NeighborAttentionBatch(nn.Module):
    ...
    def forward(self, h_V_i, h_EV_j, edge_mask):
        # h_V.sahpe [B x N x 1 x H]
        # h_E.shape [B x N x K x H]
        # return h_V.sahpe [B x N x H]
        
        B, N, K = h_EV_j.shape[:3]
        n_heads = self.num_heads
        d = int(self.num_hidden / n_heads)
        
        w = self.Bias(torch.cat([h_V_i, h_EV_j],dim=-1)).view(B, N, K, n_heads, 1) 
        attend_logits = w/np.sqrt(d) 
        attend_logits = torch.where(edge_mask[..., None, None], attend_logits, self.masked_bias)
        attend = F.softmax(attend_logits.float(), dim=2) # [B x N x K x h x H]

        V = self.W_V(h_EV_j).view(B, N, K, n_heads, d) 
        h_V = torch.sum(attend*V, 2).view([B, N, self.num_hidden])
...
class GeneralGNNBatch(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30, node_net = 'AttMLP', edge_net = 'EdgeMLP', node_context = 0, edge_context = 0):
        ...
        self.context = ContextBatch(num_hidden, num_in, num_heads=4, node_context=node_context, edge_context=edge_context)
    ...
    def forward(self, h_V, h_E, edge_idx, node_mask, edge_mask):
        ...
        h_V_i = h_V.unsqueeze(-2).expand(-1,-1, h_V_j.size(-2),-1)
        dh = self.attention(h_V_i, h_EV_j, edge_mask)
