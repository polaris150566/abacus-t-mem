...
def filter_backbone_O(structure):
    bbmask = np.zeros((len(structure),), dtype=np.bool)
    for struct in structure:
        if struct.hetero:
            bbmask = False
        elif struct.atom_name in ['N', 'CA', 'C', 'O']:
            bbmask = True
    return bbmask
...
def extract_coords_resid_from_structure(structure: biotite.structure.AtomArray):
    """
    Args:
        structure: An instance of biotite AtomArray
    Returns:
    ...
    residue_identities = get_residues(structure)[1]
    seq = ''.join([ProteinSequence.convert_letter_3to1(r) for r in residue_identities])
    resids = np.asarray([atom.res_id for atom in structure if atom.atom_name == 'CA'], dtype=np.int32)
    return coords, seq, resids
...
def load_coords_resid(fpath, chain):
    """
    Args:
        fpath: filepath to either pdb or cif file
        chain: the chain id
    ...
    """
    structure = load_structure(fpath, chain)
    return extract_coords_resid_from_structure(structure)


def load_coord_from_cif_zip(fpath, chain):
    assert(chain == 'A')
    coords = []
    residues = []
    confidences = []
    ...
    with gzip.open(fpath, 'rt') as fin:
        for line in fin:
            if line.startswith('ATOM'):
                ...
                splits = line.split()
                atom = splits[3]
                if not atom in ['N', 'CA', 'C']:
                    continue
                
                
                coord = [float(c) for c in splits[10:13]]
                coords.append(coord)

                if atom == 'CA':
                    residues.append(ProteinSequence.convert_letter_3to1(splits[5]))
                    confidences.append(float(splits[14]))
                    resids.append(int(splits[8]))

            if start and not line.startswith('ATOM'):
                break

    sequence = ''.join(residues)
    coords = np.asarray(coords, dtype=np.float32)
    coords = np.reshape(coords, (len(sequence), -1, 3))
    confidences = np.asarray(confidences, dtype=np.float32) / 100.0
    resids = np.asarray(resids, dtype=np.int32)

    return coords, sequence, resids, confidences
...
def load_confidence(confidence_path):
    with gzip.open(confidence_path, 'r') as gf:
        conf_data = json.loads(gf.read())
    return conf_data['confidenceScore']
