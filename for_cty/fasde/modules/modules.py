...
class SinusoidalPositionalEmbeddingv1(nn.Module):
    '''
    SinusoidalPositionalEmbedding input is positions
    '''
    def __init__(self, embed_dim, padding_idx, learned=False):
        super().__init__()
        self.embed_dim = embed_dim
    ...
    def forward(self, positions):
        ...
        # positions = self.make_positions(x)
        positions = torch.reshape(positions, (-1,))
        position_embedding = self.weights.index_select(0, positions).detach()
        position_embedding = torch.reshape(position_embedding, (bsz, seq_len, -1))
