...
if os.path.exists(f'{cur_dir}/.p.npy'):
    p_ = np.load(f'{cur_dir}/.p.npy')
    score_ = np.load(f'{cur_dir}/.score.npy')
else:
    print("Precomputing and saving to cache torus distribution table")
    p_ = p(x, sigma[:, None], N=100)
    np.save(f'{cur_dir}/.p.npy', p_)

    score_ = grad(x, sigma[:, None], N=100) / p_
    np.save(f'{cur_dir}/.score.npy', score_)

def score(x, sigma):
    x = (x + np.pi) % (2 * np.pi) - np.pi
    sign = np.sign(x)
    x = np.log(np.abs(x) / np.pi + 1e-10)
