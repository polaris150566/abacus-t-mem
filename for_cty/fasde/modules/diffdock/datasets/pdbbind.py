...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0,
        ...
        self.data_list = [line.strip() for line in open(split_path)]
    ...
    def get(self, idx):
        data_file = f'{self.pdbbind_dir}/{self.data_list[idx]}.npy'
        data = np.load(data_file, allow_pickle=True).item()
        del data['complex'].rec_fullatom

        if self.require_ligand:
            # complex_graph = copy.deepcopy(self.complex_graphs[idx])
            # complex_graph.mol = copy.deepcopy(self.rdkit_ligands[idx])
            complex_graph = copy.deepcopy(data['complex'])
            complex_graph.mol = copy.deepcopy(data['rdkit_ligand'])
            return complex_graph
        else:
            # return copy.deepcopy(self.complex_graphs[idx])
            return copy.deepcopy(data['complex'])
