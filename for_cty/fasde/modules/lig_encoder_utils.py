...
def flatten_graph(node_embeddings, edge_embeddings, edge_index):
    ...
    Args:
        node_embeddings: node embeddings in tuple form (scalar)
                - scalar: shape batch size x nodes x node_embed_dim
        edge_embeddings: edge embeddings of in tuple form (scalar)
    ...
    node_embeddings = torch.flatten(x_s, 0, 1)
    edge_embeddings = torch.flatten(e_s, 0, 1)
    edge_mask = torch.any(edge_index > -1, dim=1)
    # Re-number the nodes by adding batch_idx * N to each batch
#     import pdb; pdb.set_trace()
    edge_index = edge_index + (torch.arange(batch_size, device=edge_index.device) *N).unsqueeze(-1).unsqueeze(-1)
...
def flatten_graph_cross(edge_embeddings, edge_index, N1, N2):
    """
    Flattens the graph into a batch size one (with disconnected subgraphs for
    each example) to be compatible with pytorch-geometric package.
    Args:
    ...
    batch_size = e_s.shape[0]
    edge_embeddings = (torch.flatten(e_s, 0, 1), torch.flatten(e_v, 0, 1))

    edge_mask = torch.any(edge_index > -1, dim=1)
    # Re-number the nodes by adding batch_idx * N to each batch
    edge_index[:,0] = edge_index[:,0] + (torch.arange(batch_size, device=edge_index.device) * N2).unsqueeze(-1)
    edge_index[:,1] = edge_index[:,1] + (torch.arange(batch_size, device=edge_index.device) * N1).unsqueeze(-1)
