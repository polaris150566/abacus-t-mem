...
def extract_video_opencv(v_path, f_root, dim=240):
    ...
    while success:
        ...
        cv2.imwrite(os.path.join(out_dir, 'image_%05d.jpg' % count), image,
                    [int(cv2.IMWRITE_JPEG_QUALITY), 95])# quality from 0-100, 95 is default, high is good
        success, image = vidcap.read()
        count += 1
    if nb_frames > count:
        # print('/'.join(out_dir.split('/')[-2::]), 'NOT extracted successfully: %df/%df' % (count, nb_frames))
        # print_write(['/'.join(out_dir.split('/')[-2::])],"./error_extract.txt")
        pass
...
def main_UCF101(v_root, f_root):
    ...
    for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root)):
        ...
        Parallel(n_jobs=8)(delayed(extract_video_opencv)(p, f_root) for p in tqdm(v_paths, total=len(v_paths)))
...
def main_HMDB51(v_root, f_root):
    ...
    for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root)):
        v_paths = glob.glob(os.path.join(j, '*.avi'))
        v_paths = sorted(v_paths)
        Parallel(n_jobs=16)(delayed(extract_video_opencv)(p, f_root) for p in tqdm(v_paths, total=len(v_paths)))
...
def main_kinetics400(v_root, f_root, dim=150):
    ...
    for basename in ['train']:
        ...
        for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root),disable=True):
            ...
            Parallel(n_jobs=32,backend='multiprocessing')(delayed(extract_video_opencv)(p, f_root_real, dim=dim) for p in tqdm(v_paths, total=len(v_paths),disable=True)) 
            
def main_kinetics400_class(v_root, f_root, dim=150):
    ...
    for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root)):
        ...
        print('extracting: %s' % v_class)
        # dim = 150 (crop to 128 later) or 256 (crop to 224 later)
        Parallel(n_jobs=32)(delayed(extract_video_opencv)(p, f_root_real, dim=dim) for p in tqdm(v_paths, total=len(v_paths),disable=True)) 
if __name__ == '__main__':
    # v_root is the video source path, f_root is where to store frames
    # edit 'your_path' here: 
    
    # main_UCF101(v_root='/workspace/ls/data/UCF101/videos',
    #             f_root='/workspace/ls/data/UCF101/frame')

#     main_HMDB51(v_root='/workspace/ls/data/hmdb/videos',
#                 f_root='/workspace/ls/data/hmdb/frame')

#     main_kinetics400(v_root='your_path/Kinetics400/videos',
#                      f_root='your_path/Kinetics400/frame', dim=150)

    main_kinetics400(v_root='/train14/superbrain/lili27/data/uni/protein_175558/protein/videos/',
                     f_root='/train14/superbrain/lili27/data/uni/protein_175558/protein/frame', dim=256)
#     main_kinetics400(v_root='/workspace/video/data/kinetics-600/videos/',
#                      f_root='/workspace/video/data/kinetics-600/frame/', dim=256)

#     main_kinetics400(v_root='/workspace/ls/data/kinetics_400/videos/',
#                          f_root='/workspace/ls/data/kinetics_400/frames/', dim=256)
